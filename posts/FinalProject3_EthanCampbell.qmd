...
mod1 <- lm(Total_bike_users ~  Normalized_feeling_temperature_C + Normalized_Humidity + season + weekday + Hour, data = bike) 
...
I am incorporating a log here since the temperature was a positive relationship at first but as the temperature continued to increase it would then turn towards being a negative one. This is caused by bikers not wanting to be out side when the temperature gets too hot as it becomes uncomfortable and unsafe. I added a log to help account for this factor and help balance out the analysis. When looking at the R^2 we notice that compared to the original without the log this one is a good point or two higher meaning it has definitely helped explain the outcome variable. 
...
Here we will conduct the Breusch-Pagan test using the lmtest package and bptest() function. This will let us know if there is heterosexuality if the P < .05. Here we see that both meet this standard and thus we have evidence to reject the null hypothesis. Since we can reject the null we are able to say yes, heteroskedacity exists within this model. There are a few ways to go about correcting heteroskedacity the first being a robust standard error and making sure they account for the heteroskedacity. The other we can try is the generalized least squares method but for this one I am going to try the former. After applying the correction we can notice the standard error changes on each variable and we can see a different p-value. This will show us a more robust evaluation but what I have noticed is that while the significance may change a little they still remain significant in either method. Below we can see the comparison between regression 3 which is after the backward elimination and the log function and it is being compared to the robust version. 
...
correction_r1 <- coeftest(Regression1, vcov. = vcovHC(Regression1, type = "HC1"))
correction_r3 <- coeftest(Regression3, vcov. = vcovHC(Regression3, type = "HC1"))

# comparing bike regression vs the robust version
export_summs(Regression1, correction_r1, scale = TRUE,
             model.names = c("Regression(bike)", "Robust Logged Regression"))

# Comparing log bike 2 regresion against the log bike 2 robust regression
export_summs(Regression3, correction_r3, scale = TRUE,
             model.names = c("Logged Regression(bike2)", "Robust Logged Regression(bike2)"))
...
plot_summs(Regression2, Regression3,
           model.names = c("Not logged Regression", "Logged Regression"))

plot_summs(Regression1, mod1,
           model.names = c("Regression(bike)", "Regression first take(bike)"))
...
In conclusion, for hypothesis one we reject the null hypothesis with evidence of the extremely significant p-value of 2e-16. This give us evidence that we can accept the alternative and say yes temperature has an impact on bike sales. There were two different tests done here, we did the daily data compared to the hourly data. Both were significant and we controlled for 7 variables which are specified at the top. The reason we controlled these variables is that they could impact the outcome variable and thus we controlled them to make sure that they were not impacting the results. We went further and added a log to the temperature variable since we expected it to increase and then decrease. We do this to account for this change in the regression analysis and after doing so we notice the R^2 jumped for 79% to 81% so it improved how well these variables explain the outcome. After this I looked into heteroskedascity and ran it with more robust standard errors and we noticed a minor change to the variables and their standard deviations and p values. Only one variables dropped in one level of significance while the rest stayed the same this means only that one varible was not as important as it seemed originally. So in conclusion, normalized feeling temperature Celsius has an impact on bike users. Looking at our correlation graph we can see that it has a positive correlation with bike users at .631.
