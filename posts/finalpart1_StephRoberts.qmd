...
notes
from week 6 lecture video
summary(lm(price ~ new + size, data = house)

this is a multilinear regression that shows us when we control for house sie the influence of a new house on price is reduced. In other words, controlling for house sie, the "estimate" coefficient is smaller. It also tells us most new houses are larger, which means we should account for that when looking at new vs price. To check that correlation between sie and new, can use 
cor.test(house$sie, house$new)
it has a moderate positive correlation (0.38) and a very small p-value.